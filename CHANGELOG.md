...
> ⚠️ **定版时带 4 条如实边界，其中 1 条已于当日补做消除；现存 3 条**：
> 两条「未核对」（运行 A 的 `layout-audit`、运行 B 的 `qa-critic` 与 `qa-export`，因当轮走了替代路径）
> + 一条能力边界（数值审计对语义性缺失原理性失明）。
> 原「A1 的 DoD 是同一 Agent 自验」一条已由**独立盲测**补成实证（见下方「定版后补做」），
> 但该补做**不等于第三方人类评审**，也不消除其余三条。
...
定版时留下的 4 条边界里，「A1 的 DoD 是同一 Agent 自验」是**唯一可补**的一条
（另 3 条：2 条「未核对」源于当轮走了替代路径、1 条已复核证明事后不可补）。当日用**独立盲测**补成实证。
