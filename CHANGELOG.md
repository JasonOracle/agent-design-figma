...
- C3 其余三个脚本（`qa-critic` / `qa-export` / `qa-install`）——`qa-critic` 还须把 C2 的 `_evidence` 从「可选」升级为契约校验（档位自洽 / `unassessed` 为空才允许声称完整评估）· 并把 `SKILL.md` L4/L5 段「本 Skill 不随包附带校验脚本」的表述改为指向脚本
