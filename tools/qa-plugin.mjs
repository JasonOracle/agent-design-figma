...
await check("FR-5：fillOpacities 平行数组（逐位对齐 fills、未过滤；不透明=1）", () => {
  const n = makeNode("FRAME");
  n.fills = [
    { type: "SOLID", color: { r: 1, g: 0, b: 0 } },                      // 不透明 → 1
    { type: "SOLID", color: { r: 0, g: 0, b: 1 }, opacity: 0.5 },        // 半透明 → 0.5
    { type: "IMAGE", imageHash: "abc", scaleMode: "FILL" },              // 非 SOLID → 1（?? 1）
  ];
  const info = nodeInfo(n, { depth: 0 });
  assert.deepEqual(info.fillOpacities, [1, 0.5, 1], `实际 ${JSON.stringify(info.fillOpacities)}`);
  assert.deepEqual(info.fills, ["#ff0000", "#0000ff"], "fills 仍只收 SOLID 纯色且格式不变（anchor-compare 不受影响）");
  assert.equal(info.fillCount, 3, "fillCount 仍是 paint 总数");
});
...
await check("FR-3/4：TEXT 回传 lineHeight / letterSpacing（{value,unit} 原形）", () => {
  const t = makeNode("TEXT");
  const info = nodeInfo(t, { depth: 0 });
  assert.deepEqual(info.lineHeight, { value: 100, unit: "PERCENT" }, `lineHeight ${JSON.stringify(info.lineHeight)}`);
  assert.deepEqual(info.letterSpacing, { value: 0, unit: "PERCENT" }, `letterSpacing ${JSON.stringify(info.letterSpacing)}`);
  // 非 TEXT 节点不得带出这两个字段
  const f = nodeInfo(makeNode("FRAME"), { depth: 0 });
  assert.equal("lineHeight" in f, false, "FRAME 不应有 lineHeight");
});

await check("FR-2：fontWeight 数值化（Regular→400；混排/认不出→null；不猜）", () => {
  const t = makeNode("TEXT");
  assert.equal(nodeInfo(t, { depth: 0 }).fontWeight, 400, "默认 Regular 应为 400");
  t.fontName = { family: "Inter", style: "Semi Bold" };
  assert.equal(nodeInfo(t, { depth: 0 }).fontWeight, 600, "Semi Bold 应为 600");
  t.fontName = { family: "Inter", style: "Bold Italic" };
  assert.equal(nodeInfo(t, { depth: 0 }).fontWeight, 700, "Italic 后缀不影响字重");
  t.fontName = { family: "Inter", style: "Condensed Bold" };
  assert.equal(nodeInfo(t, { depth: 0 }).fontWeight, null, "认不出的 style 必须 null（不猜）");
  t.fontName = harness.figma.mixed;
  const mixed = nodeInfo(t, { depth: 0 });
  assert.equal(mixed.fontWeight, null, "混排应回 null");
  assert.equal("fontName" in mixed, false, "混排时 fontName 缺席（混排标记同源）");
});
...
await check("op 清单覆盖到 37 个（不含 run 批量壳；F2 新增 set-image-fill）", () => {
  assert.equal(opNames.length, 37, `实际 ${opNames.length}: ${opNames.join(",")}`);
  assert.ok(!opNames.includes("run"), "run 是批量壳，不计入可单独调用的 op");
  assert.ok(opNames.includes("set-effects") && opNames.includes("export-node"));
  assert.ok(opNames.includes("set-image-fill"), "F2：set-image-fill 必须在 op 面上");
});
...
await check("FR-1：ping 回传 fileKey（L5 live-build 凭证；桩给确定性值）", async () => {
  const r = await handlers.ping();
  assert.equal(r.fileKey, "harness-file-key", `ping.fileKey 实际 ${JSON.stringify(r.fileKey)}`);
});
