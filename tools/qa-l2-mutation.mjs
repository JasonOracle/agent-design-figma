...
/* ---------------- 覆盖留痕（1.3 · B3） ---------------- */
// 目录模式下，缺 DS Spec 的样例会被 QA1–QA4 **整体跳过**。若覆盖行不报，
// 「47 PASS」到底覆盖了 3 个样例还是 2 个就无从分辨（#72：没查 ≠ 查了没问题）。
{
  const covDir = path.join(TMP, "cov");
  fs.mkdirSync(covDir, { recursive: true });
  const briefText = fs.readFileSync(BRIEF, "utf8");
  fs.writeFileSync(path.join(covDir, "example-a.json"), briefText);
  fs.writeFileSync(path.join(covDir, "example-a.dsspec.json"), JSON.stringify(clone(), null, 2));
  fs.writeFileSync(path.join(covDir, "example-b.json"), briefText); // ← 故意不写 .dsspec.json
  const r = spawnSync(process.execPath, [path.join(ROOT, "tools", "qa-l2.mjs"), "--examples", covDir], {
    encoding: "utf8",
  });
  const out = `${r.stdout}${r.stderr}`;
  if (/评估覆盖/.test(out) && /缺 DS Spec 1\/2/.test(out) && /example-b/.test(out)) {
    console.log("  ok   样例缺 spec 时覆盖行报出数量与名字（不静默跳过）");
    pass++;
  } else {
    console.log(`  MISS 覆盖行未报出缺 spec 样例\n${out.split("\n").slice(0, 6).join("\n")}`);
    fail++;
  }
}
