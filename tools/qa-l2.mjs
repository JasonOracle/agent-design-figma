...
/**
 * 「查了几个样例」必须可见：`pairs` 里 `spec` 为 null 的样例会被 QA1–QA4 依次 `continue`
 * **整体跳过**，而汇总里的 PASS 数看不出覆盖的是 3 个样例还是 2 个（#72：没查 ≠ 查了没问题）。
 */
const coverageRows = [];
coverageRows.push(`样例 ${pairs.length} 个：${pairs.map((p) => p.name).join("、") || "无"}`);
const noSpecPairs = pairs.filter((p) => !p.spec).map((p) => p.name);
const noBriefPairs = pairs.filter((p) => p.spec && !p.brief).map((p) => p.name);
if (noSpecPairs.length) {
  coverageRows.push(
    `⚠ 缺 DS Spec ${noSpecPairs.length}/${pairs.length} → QA1–QA4 对其**全部未执行**：${noSpecPairs.join("、")}`,
  );
}
if (noBriefPairs.length) {
  coverageRows.push(`⚠ 缺同级 Brief ${noBriefPairs.length} 个 → 依赖 Brief 的交叉校验未执行：${noBriefPairs.join("、")}`);
}
