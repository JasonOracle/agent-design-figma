...
4. **连接插件**：在 Figma 中打开任意设计文件 → Plugins → Development → **agent-design-figma Bridge (Dev) · F2b** → 把 token 粘贴进插件面板 → Connect。面板显示已连接即成功。

> 为什么需要 token：Bridge 只监听本机回环地址，token 防止其他本地进程误用你的 Figma 连接。默认启动方式下 token 跨重启保持不变（存放在**仓库目录**下自动生成的 `.vibe/token` 文件中，与你在哪个目录敲命令无关）。
>
> ⚠️ **以启动日志打印的 token 为准**。若启动时带了 `--token <值>` 或环境变量 `VIBE_TOKEN`（测试/临时隔离场景才会用到），该值**直接生效但不写入 `.vibe/token`**——此时文件里是旧值，照它连会失败。日常使用不需要这两个参数。
...
| 修改了插件代码不生效 | Figma 插件不会热更新：Plugins → Development → 重新运行 agent-design-figma Bridge (Dev) · F2b |
| 改了 code.js 重跑还是旧代码 | Figma 按 manifest id 缓存开发插件包，重启也不重读：改一下 manifest（如名字加后缀）后 **Import plugin from manifest** 重新导入，插件窗口日志的 ops 清单是版本指纹 |
