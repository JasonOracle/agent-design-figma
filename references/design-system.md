...
| **ST-1** | status 四色 = 从 `preset.chartColors` **按色相分类映射**（绿系→success、黄/橙系→warning、红系→danger、蓝/青系→info），**不改值不混色**。**黄系与橙系并存时**（H4 三轮盲区）：取**数组顺序第一个**命中项——判据是「先到先得」，可复现、不靠语感；并在 `sourceMapping.assumptions` 记一句「warning 取 `<hex>`（黄/橙并存，按序首个）」 | `color.status.*` |
...
| **TY-5** | **字阶串 → 五类 token 的映射**（H1 三轮盲区，各猜一套的根源）。Brief/preset 的 `typography` 字阶串是**段序列**（如 `26-32-16-24-14-20-13-20-12-18-11-16`），逐档贪心消费，规则唯一：① 维护剩余段队列，对 display → heading → body → caption 依次取档：队首两段 `(a,b)` 满足 **`b > a`**（行高必大于字号）→ 消费两段为「字号-行高对」；否则只消费一段为字号，行高按 TY-2 推。② number 档**恒复用 body 档字号**（TY-1 原文），字体按 TY-1 换等宽/Inter；四档取完后若队列仍剩**合法对**（`b > a`），取该对为数字强调档覆盖（`source` 留痕）。③ 取完仍有剩余段 → `sourceMapping.assumptions` 记「富余段 ×N：<值>」，**不静默丢弃**。④ 每档 `source` 写明取自哪几段（如 `preset:…typography(26-32)` / `typography(17)+rule:TY-2`）。few-shot 对照：`example-health`（24-32-17-15-13-11）被本规则**逐档复现**；`example-saas` 的 number 档取了 heading 对（16-24），属收紧前的自由度，历史产物不改、新产出按本条 |
| **TY-6** | **字重取值**（H6 盲区）：标题（display/heading）**500**、正文与 caption **400**、number **500**（等宽强调）。允许区间：标题 500–600（营销 Hero 可 700，须记 `assumptions`）；正文恒 400——**禁 300 以下细体、禁整段 ≥500**。标题与正文同字重时层级不可辨，故**同级差 ≥100** 为硬下限 |
...
| **SP-4** | **其余 spacing 档位缺省与允许区间**（H2 三轮盲区：SP-2/SP-3 之外无取值规则）：`spacing.base` = **8**（4–12）；web-admin / web-site `pagePadding` = **24**（16–32）；`cardPadding` = **16**（12–24）；区块纵向间距 = **24**（16–40）。一律 **4 的倍数**（8 栅格及其半档）；取值 `source` 记 `rule:SP-4`，出区间须记 `assumptions` 并说明由 |
| **AC-3** | **focusRing 缺省与区间**（H5 盲区；正式化 `example-saas` 已引用的 `rule:AC-3`）：`2px <color.brand.primary>`、offset **2px**。允许区间：宽 **1–3px**、offset **0–4px**；颜色改用 `color.status.info` 时记 `assumptions` |
| **AC-4** | **touchTarget 缺省与区间**（H5 盲区）：桌面指针设备 **≥30px**（`preset.geometry.button` 已给值则按 preset，如 example-saas = 30）；**触屏设备 ≥44px**（允许 40–48；gov / big-screen ≥48）。设备类型由 Brief 平台判定并留痕 |
...
| **LD-10** | **大屏 `frameBaseline` 恒为 1920×1080**，3840×2160 **只是 @2x 出图倍率、不进 `frameBaseline`**。任何平台下 `frameBaseline` 都记**设计层逻辑像素**，出图倍率写在 `notes` 或 `sourceMapping.assumptions` 里。用户若按「大屏 3840×2160」提需求，**默认理解为出图尺寸**——除非用户明确说「按 1:1 物理像素渲染、不要缩放」（见下） |
| **LD-11** | **web-admin 响应式断点**（H3 三轮盲区：四平台里唯 web-admin 未定）：缺省两档 **1440** / **1024**。≥1440 满档（§11.1 模板原样，KPI 4 列）；1024–1439 半档（KPI 降 2 列、Sidebar 折叠至 64）；<1024 降档（Sidebar 抽屉化、多列降单列、表格横向滚动）。Brief 显式给断点时覆盖并记 `briefOverrides`。其余平台：mobile-app / big-screen 固定基线无响应式档，web-site 见 LD-9 |
| **GEO-1** | 分区尺寸取整到 px（如 1080×8% ≈ 86） |

> **大屏的 1920 vs 3840：为什么必须以 1920 为设计基准。**
> 大屏的设计意图是「**3–10 米观看距离**下远可辨状态、近可查明细」，所有的字号下限
> （正文 ≥14px）与信息密度（`dense`、图表占比 ≥50%）都是**针对 1920×1080 这个逻辑画布**定的。
> 3840×2160 是同一张设计在 4K 屏上的**清晰度倍率**——视觉大小不变，只是像素更密。
> **若把 3840 误当真基线，14px 会变成 28px、间距全部翻倍**，结果是远看字大得离谱、
> 一屏塞不下本该有的信息量，与「数据即装饰、密集可读」的意图正相反。
>
> **唯一的例外**：用户明确要求「**按物理像素 1:1 输出、不做缩放渲染**」（典型场景是
> **LED 拼接屏按点对点出图**）。此时才把 `frameBaseline` 记为真实物理分辨率，
> 且 **L2 的 `typography` 与 `spacing` 必须整体按倍率重推**——这是一次**规则切换**，
> 不是改一个字段，必须在 `sourceMapping.assumptions` 里写明并按新基准重跑 `qa-l2`。
