...
证据来源按**环境能力**分两档。核心纪律一句话：**读图是增益，不是前提**。

| 档 | 谁有 | 证据形态 | 与评分的关系 |
|---|---|---|---|
| **通用** | 所有环境（只要 Bridge 能 `get-node`） | 结构化回读的实测值（坐标 / 尺寸 / fill / 字号 / 文本内容 / 节点命名） | **必进**，且恒须产出 |
| **增益** | 只有**能读图的 Agent**（具备多模态读 PNG 的能力） | 导出 PNG + 逐条观察（每条须写「看到了什么 + 对应节点或区域 + 影响哪一维」） | 附加，可上调分值，**不得替代**通用档 |
...
| Commercial（§1.4） | 模板感（结构化代理：`Frame 123` 式默认命名、lorem 文本、`#D9D9D9` 占位块）· 品牌识别（primary 出现处计数）· 细节完成度（状态变体与阴影 token 是否成体系） | **商业产品相似度（40%）**：与锚点产品并置比对只能读图；模板感的"观感"部分 |
...
2. **增益档（能读图才用）**：导出 PNG 供多模态复核。Commercial 的「商业产品相似度」与三维的观感项属于此档。增益档的观察必须写成**可回放的条目**（看到了什么 + 对应节点/区域 + 影响哪一维），且**只能上调分值或补充发现，不得替代通用档**；`_evidence.mode` 据实填 `structured-only` 或 `structured+vision`；
