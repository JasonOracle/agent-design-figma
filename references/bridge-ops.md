...
共 **37 个 op**（另有 `run` 批量壳）。分组排列。

> **asset 端点（F2 配套）**：`GET /v1/asset?name=<file>&token=…` —— 从 `<repo>/.vibe/assets/`
> 取图片字节（扩展名白名单 png / jpg / jpeg / webp，上限 8MB，只收纯文件名防穿越）。
> 这是插件 UI 拉图的**唯一**同域通道（`manifest.allowedDomains` 只有 localhost:45677，不改 manifest）。
...
| `set-image-fill` | `id`, `asset`（Bridge asset 目录 `.vibe/assets/` 里的**纯文件名**）, `scaleMode?`（`FILL`\|`FIT`\|`CROP`\|`TILE`，缺省 `FILL`）, `opacity?`（0–1）。字节链路：主线程 → UI 同域 `GET /v1/asset` fetch → base64 回传 → `figma.createImage`。回读核验走 `imageFills` 字段（`image:<hash>:<scaleMode>`） |
